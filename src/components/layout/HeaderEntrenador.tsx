...
const DROPDOWN_ENTRENADOR = [
    ...
    { href: '/dashboard/entrenador/plantilla', label: 'Asistencias' },
    { href: '/dashboard/entrenador/plantilla', label: 'Convocatorias' },
    { href: '/dashboard/entrenador/estadisticas', label: 'Evaluación Jugadores' },
    { href: '/dashboard/entrenador/plantilla', label: 'Lesiones' },
]
    ...
    return (
        ...
        >
            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
                ...
                {/* Nav — mismos links del sitio público */}
                <nav className="hidden items-center gap-0.5 md:flex">
                    {NAV_LINKS.map(({ href, label }) => (
                        <Link
                            key={href}
                            href={href}
                            className={`rounded-lg px-3.5 py-2 text-sm font-medium transition-all duration-150 ${pathname === href
                    ...
                    <Link
                        href="/notificaciones"
                        className={`rounded-lg px-3.5 py-2 text-sm font-medium transition-all duration-150 ${pathname === '/notificaciones'
                                ? 'bg-white text-red-600 shadow-sm'
                                : 'text-white/90 hover:bg-white/15 hover:text-white'
                            }`}
                    >
                    ...
                    <div className="relative" ref={dropdownRef}>
                        <button
                            ...
                            className={`flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium transition-all duration-150 ${pathname.startsWith('/dashboard/entrenador')
                                    ? 'bg-white text-red-600 shadow-sm'
                                    : 'text-white/90 hover:bg-white/15 hover:text-white'
                                }`}
                        >
                            ...
                            Entrenador
                            <svg
                                className={`w-3.5 h-3.5 transition-transform duration-150 ${dropdown ? 'rotate-180' : ''}`}
                        ...
                        {/* Menú desplegable */}
                        {dropdown && (
                            <div className="absolute left-0 mt-2 w-52 rounded-xl border border-gray-100 bg-white shadow-lg overflow-hidden z-50">
                                {DROPDOWN_ENTRENADOR.map(({ href, label }) => (
                                    <Link
                                        key={label}
                                        href={href}
                                        onClick={() => setDropdown(false)}
                                        className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors duration-100"
                                    >
                                        {label}
                                    </Link>
                                ))}
